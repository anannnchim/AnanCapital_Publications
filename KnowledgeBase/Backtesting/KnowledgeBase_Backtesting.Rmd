...
-   3.3 Market checking (empirical)

    -   Use other model to represent price: try different model.(Need to simulate if using random data)
    -   Change in regime: change in volatility and return.
...
-   3.5 Strategy comparing

    -   inverse signal

    -   Benchmark: B&H and random.

    -   Use SMA to check robustness.

-   Summary on how to fail.

    -   If parameter change by X%

    -   If lag signal by Xday

    -   if cost increase by X%
